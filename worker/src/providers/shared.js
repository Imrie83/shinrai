...
IMPORTANT:
- Elements labelled "Section" group a heading with its immediately following content (e.g. "[H2] Bathrooms | [p] Spacious modern bathrooms with..."). Treat the entire Section as one logical unit -- do not flag missing content that is already present within the same Section entry.
- Treat all other labelled entries as individual elements.
...
{
  "score": <integer 0-100>,
  "summary": "<2-3 sentence overview, written in an encouraging and constructive tone. Acknowledge what the site does well, then briefly note the main areas for improvement. Be specific but kind — the reader is a Japanese business owner, not a developer. Never use harsh language or imply the site is bad. Focus on opportunity, not failure.>",
...
{
  "score": <整数0〜100>,
  "summary": "<2〜3文で要約。励ましと建設的なトーンで記述し、サイトの良い点を認めつつ、改善できる主なポイントを前向きに伝えること。批判的・否定的な表現は避け、可能性や機会として表現すること（日本語で）>",
